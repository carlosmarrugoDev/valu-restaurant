...
export async function GET(req: NextRequest) {
  try {
    ...
    const { data: pedidos } = await supabase
      ...
      .in('estado', ['en_cocina', 'listo', 'en_preparacion'])
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (item_id) {
      if (!pedido_id) {
        return NextResponse.json({ error: 'pedido_id requerido para actualizar un item' }, { status: 400 })
      }
    ...
    if (pedido_id) {
      ...
      return NextResponse.json({ success: true, message: 'Pedido marcado como listo', estado: 'listo' })
    }
